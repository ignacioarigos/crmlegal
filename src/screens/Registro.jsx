...
export function Registro({ store }) {
  ...
  const handleSave = async () => {
    ...
    const obj = { 
      ...
      vencimiento_fecha: tieneVenc && vencResult ? vencResult.fecha : null, 
      vencimiento_texto: tieneVenc && vencResult ? vencResult.texto : null, 
    ...
    if (crearTarea) {
      ...
      await saveTarea({ 
        ...
        titulo: tt, 
        causa: obj.causa, 
        criticidad: tieneVenc && vencResult ? 'urgente' : 'normal', 
        vencimiento: tieneVenc && vencResult ? vencResult.fecha : null, 
        estado: 'no-iniciada', 
        notas: estrategia.trim() || null, 
        fecha: new Date().toISOString() 
      })
    }
  ...
  const resetForm = () => { 
    setPortal(''); setCausaId(''); setNovedad(''); setEstrategia(''); 
    setTieneVenc(false); setDias(''); setFechaInicio(dateFmt(new Date())); 
    setVencResult(null); setCrearTarea(false); setTareaTitulo(''); setCalcMsg('');
    setBusqueda(''); setMostrarSug(false);
  }
        ...
        <Modal title="Nueva Novedad" onClose={() => { setModal(false); resetForm() }}>
          <div className="form-row">
            <div className="form-group">
              <label>Portal *</label>
              <select className="form-control" value={portal} onChange={e => { setPortal(e.target.value); setCalcMsg('') }}>
                <option value="">— Seleccione —</option>
                <option>PJN</option>
                <option>SCBA</option>
                <option value="EJE">EJE (CABA)</option>
              </select>
            </div>
            ...
            <div className="form-group" style={{ position: 'relative' }}>
              ...
              {causaId && <div style={{fontSize:'.7rem', color:'var(--primary)', marginTop:'2px'}}>✓ Seleccionado</div>}
            </div>
          </div>

          <div className="form-group" style={{ marginBottom: '.9rem' }}>
            <label>Novedad / Providencia *</label>
            <textarea className="form-control" rows="3" value={novedad} onChange={e => setNovedad(e.target.value)} placeholder="Describa el movimiento..." />
          </div>

          <div className="form-group" style={{ marginBottom: '.9rem' }}>
            <label>Estrategia / Respuesta</label>
            <textarea className="form-control" rows="2" value={estrategia} onChange={e => setEstrategia(e.target.value)} placeholder="¿Qué acción plantea?" />
          </div>
          ...
          <div className="card" style={{ background: 'var(--cream)', marginBottom: '.9rem', padding: '.9rem' }}>
            <div className="checkbox-row" style={{ marginBottom: '.65rem' }}>
              <input type="checkbox" checked={tieneVenc} onChange={e => setTieneVenc(e.target.checked)} />
              <label><strong>Vencimiento de plazo</strong></label>
            </div>
            {tieneVenc && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', flexWrap: 'wrap' }}>
                  <select className="form-control" value={dias} onChange={e => { setDias(e.target.value); setVencResult(null) }} style={{ width: 80 }}>
          ...
          <div className="card" style={{ background: 'var(--cream)', marginBottom: '.9rem', padding: '.9rem' }}>
            <div className="checkbox-row">
              ...
              <label><strong>Generar tarea a partir de este registro</strong></label>
            </div>
            {crearTarea && (
              <div style={{ marginTop: '.75rem' }}>
                <div className="form-group">
                  <label>Título de la tarea</label>
                  <input className="form-control" value={tareaTitulo} onChange={e => setTareaTitulo(e.target.value)} placeholder="Si vacío, se usa el texto de la novedad" />
                </div>
              </div>
            )}
          </div>
